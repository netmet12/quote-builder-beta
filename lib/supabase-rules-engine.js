...
export class SupabaseRulesEngine {
  ...
  async getVisibleSections(productId, selections) {
    // Debug when "Has wall been built" = No (section 11, option 290)
    const isWallNotBuilt = selections['11'] && selections['11'].includes(290)
    
    console.log('🔍 Rules Engine getVisibleSections called')
    console.log('Product ID:', productId)
    console.log('Wall not built scenario:', isWallNotBuilt)
    console.log('All selections:', selections)
    
    try {
      const allSections = await supabaseDataService.getSectionsForProduct(productId)
      console.log(`📋 Retrieved ${allSections.length} total sections from database`)
      ...
      for (const section of allSections) {
        try {
          ...
          // Log all sections when wall not built, or just section 21 normally
          if (isWallNotBuilt || section.id === 21) {
            console.log(`Section ${section.id} "${section.title}": ${isVisible ? '✅ VISIBLE' : '❌ HIDDEN'}`)
          }
      ...
      console.log(`🎯 Final result: ${visibleSections.length} visible sections`)
      return visibleSections
      
    } catch (error) {
      console.error('🚨 CRITICAL ERROR in getVisibleSections:', error)
      return []
    }
  }
