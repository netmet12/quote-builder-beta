...
export function validateSelections(sections, selections) {
  ...
  // In the original Vue.js app, users had to make selections to proceed
...
export function getNextStep(currentStep, sections, selections, productId) {
  for (let i = currentStep + 1; i < sections.length; i++) {
    const section = sections[i]
    
    // Check if this section should be visible using rules engine
    if (rulesEngine.isSectionVisible(section.id, selections, productId)) {
      return i
    }
  }
...
export function getPrevStep(currentStep, sections, selections, productId) {
...
export function getVisibleSections(productId, selections) {
  return rulesEngine.getVisibleSections(productId, selections)
}

// Calculate quote completeness
export function getQuoteCompleteness(sections, selections) {
  // For now, treat all sections as required
...
export function formatOptionsForDisplay(section, selections, productId) {
  const visibleOptionIds = rulesEngine.getVisibleOptions(section.id, selections, productId)
  ...
  return optionsToShow
    ...
    .map(id => ({
      ...
      popular: section.options[id].is_most_popular === 1,
      customInput: section.options[id].requires_input === 1,
